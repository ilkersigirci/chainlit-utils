...
async def _after_thread_hydration(
    ...
    # Chainlit sends resume_thread only after on_chat_resume returns and offers no
    # hydrated callback, so live UI must be emitted on the next short turn.
...
__all__ = ["reuse_persisted_step", "schedule_after_thread_hydration"]
